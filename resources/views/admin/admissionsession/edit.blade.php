    ...
    <form action="{{ route('admin.academic-sessions.update', $session->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="session_type" class="form-control" required>
                <option value="fall"   {{ old('session_type', $session->session_type) == 'fall' ? 'selected' : '' }}>Fall</option>
                <option value="spring" {{ old('session_type', $session->session_type) == 'spring' ? 'selected' : '' }}>Spring</option>
                <option value="summer" {{ old('session_type', $session->session_type) == 'summer' ? 'selected' : '' }}>Summer</option>
            </select>
        </div>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="date" name="admissions_open_date"
                       value="{{ old('admissions_open_date', $session->admissions_open_date?->format('Y-m-d')) }}"
                       class="form-control">
            </div>

            <div class="col-md-6 mb-3">
                <label class="form-label">Admissions Close Date</label>
                <input type="date" name="admissions_close_date"
                       value="{{ old('admissions_close_date', $session->admissions_close_date?->format('Y-m-d')) }}"
                       class="form-control">
            </div>
        </div>
        ...
        <div class="mb-3 form-check">
            ...
            <input type="checkbox" name="is_admissions_open" value="1" id="is_admissions_open" class="form-check-input"
                {{ old('is_admissions_open', $session->is_admissions_open) ? 'checked' : '' }}>
